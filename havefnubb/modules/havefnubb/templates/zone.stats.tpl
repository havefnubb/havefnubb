...
<div class="box">
    ...
    <ul>
        ...
        <li>{@havefnubb~main.last.post@} : <a href="{jurl 'havefnubb~posts:viewtogo',array('id_post'=>$lastPost->id_first_msg,'thread_id'=>$lastPost->thread_id,'id_forum'=>$lastPost->id_forum,'ptitle'=>$lastPost->subject,'ftitle'=>$forum->forum_name,'go'=>$lastPost->id_last_msg)}#p{$lastPost->id_last_msg}" title="{@havefnubb~main.goto_this_message@}">{$lastPost->subject|eschtml}</a></li>
