...
class forumCtrl extends jController {
    ...
    */
    ...
    function view() {
		...
		$GLOBALS['gJCoord']->getPlugin('history')->change('label', htmlentities($forum->forum_name) . ' - ' . jLocale::get('forum.page') . ' ' .($page+1));
		
        $rep = $this->getResponse('html');
        if ($page == 0)
            $rep->title = $forum->forum_name;
        else
            $rep->title = $forum->forum_name . ' - ' . jLocale::get('forum.page') . ' ' .($page+1) ;
        $tpl = new jTpl();
