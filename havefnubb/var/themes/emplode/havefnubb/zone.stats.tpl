...
<div id="stats">
    ...
    <ul>
        ...
        <li>{@havefnubb~main.last.post@} : <a href="{jurl 'havefnubb~posts:view',array('id_post'=>$lastPost->id_post,'thread_id'=>$lastPost->thread_id,'id_forum'=>$lastPost->id_forum,'ptitle'=>$lastPost->subject,'ftitle'=>$forum->forum_name)}#p{$lastPost->id_post}" title="{@havefnubb~main.goto_this_message@}">{$lastPost->subject|eschtml}</a></li>
