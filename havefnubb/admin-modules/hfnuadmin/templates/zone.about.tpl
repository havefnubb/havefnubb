<table class="records-list" width="98%">
    ...
    <tbody>
        ...
        {/foreach}
        <tr>
            <td>{@hfnuadmin~hfnuabout.about.links@}</td><td><a href="{$moduleInfo['homepageURL']}" title="{@hfnuadmin~hfnuabout.about.homepageURL@} : {$moduleInfo['name']}">{@hfnuadmin~hfnuabout.about.homepageURL@}</a>{if $moduleInfo['updateURL'] != ''} - <a href="{$moduleInfo['updateURL']}" title="{@hfnuadmin~hfnuabout.about.updateURL@} : {$moduleInfo['name']}">{@hfnuadmin~hfnuabout.about.updateURL@}</a>{/if}</td>
        </tr>
    </tbody>
</table>
