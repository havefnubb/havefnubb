...
<div class="box">
    ...
    {for $i = 0 ; $i < count($datas) ; $i++ }
        <div class="result-line">
            ...
            <span class="result-author">{@havefnubb~post.heading.postedby@} {$datas[$i]['login']}</span> - <span class="result-date">{$datas[$i]['date_created']|jdatetime:'timestamp':'lang_datetime'}</span> <br/>
            <div class="result-msg">{$datas[$i]['message']|wiki|stripslashes}</div>
        </div>
    {/for}
    {pagelinks 'hfnusearch~default:query', array('hfnu_q'=>$string,'perform_search_in'=>$perform_search_in),
                $count, $page, $resultsPerPage, "page", $properties}
